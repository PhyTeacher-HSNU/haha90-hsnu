...
//B.定義init
function init(){
	hand = new TEACHER.ObjPicPlane(80,50,pics.hand);
	scene.add(hand);
	
    world2D.ch01.setLabel('草地');
	world2D.ch02.setLabel('柏油路');
	world2D.sl01.setLabel('力道');
	world2D.sl01.minimum=0;
	world2D.sl01.maximum=5;
	world2D.sl01.digitN=1;
	world2D.sl01.value=3;

	world2D.btn01.setLabel('重新開始');
	world2D.btn01.on('click',clickBtn);
	

	grass=new TEACHER.ObjPicPlane(100,100,pics.grass,'y');
	road=new TEACHER.ObjPicPlane(100,100,pics.road,'y');


	hand.position.y=7;
	hand.position.x=0;
	
	win = new TEACHER.ObjPicPlane(20,15,pics.win);
	win.position.y=20;
	
	lose = new TEACHER.ObjPicPlane(20,15,pics.lose);
	lose.position.y=20;
...
function tick(){
	...
	ground.visible=!world2D.ch01.checked;
	ground.visible=!world2D.ch02.checked;
	world3D.scene.add(grass);
	world3D.scene.add(road);
	grass.visible=world2D.ch01.checked && !world2D.ch02.checked;
	road.visible=world2D.ch02.checked && !world2D.ch01.checked;
	vv=world2D.sl01.value;
	
	

	if(world2D.ch01.checked){
		hand.position.x+=0.5*(vv-1.5);
	}else if(!world2D.ch01.checked){
	    hand.position.x+=0.5*(vv-3);
	}

	if(world2D.ch02.checked){
		hand.position.x+=0.5*(vv-0.1);
	}else if(!world2D.ch02.checked){
	    hand.position.x+=0.5*(vv-3);
	}

	if(world2D.ch01.checked && world2D.ch02.checked){
		hand.position.x=0;
	}

	if(hand.position.x>50){
		hand.position.x=50;
		scene.add(win);
		win.visible=true;
	}else if(hand.position.x<-50){
		hand.position.x=-50;
		scene.add(lose);
		lose.visible=true;
	}else if(-50<hand.position.x<50){
		lose.visible=false;
		win.visible=false;
	}
