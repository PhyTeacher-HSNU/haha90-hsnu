...
function init(){
	...
	aa=new TEACHER.ObjPicPlane(50,50,pics.mon1,"z",0);
	scene.add(aa);
	aa.scale.x = 0.3
	aa.scale.y = 0.4
	aa.scale.z
	...
	bb=new TEACHER.ObjPicPlane(50,50,pics.mon2,"z",0);
	scene.add(bb);
	bb.scale.x = 0.3
	bb.scale.y = 0.4
	bb.scale.z
	...
	cc=new TEACHER.ObjPicPlane(50,50,pics.bro1,"z",0);
	scene.add(cc);
	cc.scale.x = 0.3
	cc.scale.y = 0.4
	cc.scale.z
	...
	dd=new TEACHER.ObjPicPlane(50,50,pics.bro2,"z",0);
	scene.add(dd);
	dd.scale.x = 0.3
	dd.scale.y = 0.4
	dd.scale.z
	...
	ee=new TEACHER.ObjPicPlane(50,50,pics.mon3,"z",1);
	scene.add(ee);
	ee.scale.x = 0.5
	ee.scale.y = 0.5
	ee.scale.z
	...
	ff=new TEACHER.ObjPicPlane(50,50,pics.bro3,"z",1);
	scene.add(ff);
	ff.scale.x = 0.5
	ff.scale.y = 0.5
	ff.scale.z
	...
	go=new TEACHER.ObjPicPlane(15,15,pics.good,"z",2);
	scene.add(go);
	go.position.x = 0
	go.position.y = 13
	go.position.z = -45

	s=new TEACHER.ObjPicPlane(16,9,pics.strike,"z",2);
	scene.add(s);
	s.position.x = 35
	s.position.y = 15
	s.position.z = -45
	...
	b=new TEACHER.ObjPicPlane(16,9,pics.bad,"z",2);
	scene.add(b);
	b.position.x = 35
	b.position.y = 15
	b.position.z = -45
	...
	world2D.btn02.setLabel('側投球');

	world2D.btn01.on('click',clickBtn);
	world2D.btn02.on('click',clickBtn);

	world2D.ch01.setLabel('好球帶');
	world2D.ch02.setLabel('sl左方有說明');

	world2D.sl01.setLabel('前2球不算');
	world2D.sl02.setLabel('投球間隔2秒以上');
	world2D.sl03.setLabel('較大幅度轉彎請連按方向鍵');
...
function tick(){
		...
		if(ball.position.z<-50){
			...
			if(ball.position.z<-50){
				if(ball.position.x>-10 && ball.position.x<10){
					if(ball.position.y>4 && ball.position.y<21){
						s.visible=true
						b.visible=false
					}else{
						b.visible=true
	...
	go.visible=world2D.ch01.checked;
	logo.visible=world2D.ch01.checked;
...
var logo=new TEACHER.ObjPicPlane(100,100/8,pics.logo,'z',0);
logo.position.z=-50;
