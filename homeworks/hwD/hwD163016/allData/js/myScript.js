...
var ball,bar,center,dec1,dec2,wall,arrow1
var arrowV
var dt=0.1
var rr
var omega=0.2
...
function init(){
	//功能鍵
	world2D.slCameraRR.setLabel('發現最好時機,呈現最深天機(建議多加嘗試各功能)///[已擊中0/5:正]')
	world2D.slCameraRR.value=150

	world2D.sl01.setLabel('中心高度')
	world2D.sl01.minimum=2.5
	world2D.sl01.maximum=47.5
	world2D.sl01.value=25
	world2D.sl01.digitN=1
	world2D.sl02.setLabel('影響幅度')
	world2D.sl02.minimum=10
	world2D.sl02.maximum=40
	world2D.sl02.value=20
	world2D.sl02.digitN=0
	world2D.sl03.setLabel('轉動速度')
	world2D.sl03.minimum=-5
	world2D.sl03.maximum=5
	world2D.sl03.value=1
	world2D.sl03.digitN=1

	world2D.ch01.setLabel('困難(調整部分功能)')
	world2D.ch01.checked=false
	world2D.ch02.setLabel('龍捲')
	world2D.ch01.checked=false
	world2D.btn01.setLabel('電神,啟動')
	world2D.btn02.setLabel('獄門旋,開')
	...
	//加入物件
	ball=new TEACHER.ObjSphere(2.5,0xF0F0F0)
	bar=new TEACHER.ObjCylinder(0.25,world2D.sl02.value,0x000001,true,"x")
	center=new TEACHER.ObjCylinder(0.25,45,0x000001)
	dec1=new TEACHER.ObjCylinder(0.5,2.5,0xFFFFFF)
	dec2=new TEACHER.ObjCylinder(0.5,2.5,0xFFFFFF)
	wall=new TEACHER.ObjPlane(400,50,0x000001,"z",2)
	arrow1=new TEACHER.ObjArrow(1,0xFF00FF)
	scene.add(ball).add(bar).add(center).add(dec1).add(dec2).add(wall).add(arrow1)
	
	arrowV=new TEACHER.ObjArrow(0.5,0xFF00FF)
	ball.add(arrowV)
	...
	for(var i=0;i<n;i+=1){
		if(i%2===0){
			...
			var bar1=new TEACHER.ObjCylinder(0.25,i/2,0xFF0000,true,"x")
			var bar3=new TEACHER.ObjCylinder(0.25,i/2,0xFF0000,true,"x")
			...
			var bar2=new TEACHER.ObjCylinder(0.25,i/2,0xFFFFFF,true,"x")
			var bar4=new TEACHER.ObjCylinder(0.25,i/2,0xFFFFFF,true,"x")	
	...
	center.position.z=50

	//dec1,dec2
	dec1.position.y=1.25
	dec1.position.z=50
	dec2.position.y=48.75
	dec2.position.z=50
	//wall,arrow1
	wall.position.y=25
	wall.position.z=-50

	arrow1.position.y=25
	arrow1.position.z=-50
	arrow1.scale.x=10
	arrow1.scale.y=0.25
	arrow1.scale.z=10
	arrow1.visible=false//第一層配置
...
function tick(){
	...
	if(world2D.ch02.checked===true){
		...
		for(var i=0;i<n;i+=1){
			...
			//bao
			bao.position.x=i/2*Math.cos(i*2*theta)+X
			bao.position.z=i/2*Math.sin(i*2*theta)+Z
			//bat
			bat.position.x=-i/2*Math.cos(i*2*theta)+X
			bat.position.z=-i/2*Math.sin(i*2*theta)+Z
			//baro
			baro.position.x=0.5*i/2*Math.cos(i*2*theta)+X
			baro.position.z=0.5*i/2*Math.sin(i*2*theta)+Z
			baro.rotation.y=-theta
			//bart
			bart.position.x=-0.5*i/2*Math.cos(i*2*theta)+X
			bart.position.z=-0.5*i/2*Math.sin(i*2*theta)+Z
		...
		for(var i=0;i<n;i+=1){
			...
			var baro=baroArray[i]
			var bart=bartArray[i]
			bao.position.x=bao.position.z=bat.position.x=bat.position.z=0
			baro.position.x=baro.position.z=bart.position.x=bart.position.z=0
			bao.visible=bat.visible=baro.visible=bart.visible=false
		}
	}

	var rr=world2D.sl02.value
	//鎖定(ch01)
	if(world2D.ch01.checked===true){
		...
		if(world2D.slCameraRR.value>=150 && upca>0){
			upca*=-1
		}else if(world2D.slCameraRR.value<=50 && upca<0){
			upca*=-1
		}
		world2D.sl01.minimum=12.5
	    world2D.sl01.maximum=37.5
	...
	if(world2D.ch01.checked===true || world2D.ch02.checked===true){
		...
		dec1.visible=false
		dec2.visible=false
	}else if(world2D.ch01.checked===false || world2D.ch02.checked===false){
		ball.visible=true
		bar.visible=true
		arrowV.visible=true
		center.visible=true
	...
	if(Th===0){
		...
		ball.position.z=rr*Math.sin(theta)+50
		ball.position.y=world2D.sl01.value
	}else if(Th===1){
		if(world2D.sl03.value>0){
		...
		}else if(world2D.sl03.value<0){
			ball.position.x+=vv*Math.sin(theta)
			ball.position.z-=vv*Math.cos(theta)
		}
	}else if(Th===10){
		ball.position.x+=vv*world2D.sl03.value/2.5
		ball.position.z+=vv*world2D.sl03.value/2.5
	}else if(Th===20){
		ball.position.x+=vv*world2D.sl03.value/2.5
		ball.position.z-=vv*world2D.sl03.value/2.5
	}else if(Th===30){
	...
	//btn01擊破>=
	if(SC===0 && ball.position.x>=-200 && ball.position.x<=200 && ball.position.z<=-50){
		SC+=1//第二層配置
		Th=0
		...
		wall.scale.x=0.5
		world2D.slCameraRR.setLabel('發現最好時機,呈現最深天機(建議多加嘗試各功能)///[已擊中1/5:下]')
	}else if(SC===1 && ball.position.x>=-100 && ball.position.x<=100 && ball.position.y<=25 && ball.position.z<=-50){
		SC+=1//第三層配置
		Th=0
		arrow1.rotation.x=Math.PI*2
		wall.scale.x=0.5
		world2D.slCameraRR.setLabel('發現最好時機,呈現最深天機(建議多加嘗試各功能)///[已擊中2/5:上]')
	}else if(SC===2 && ball.position.x>-100 && ball.position.x<=100 && ball.position.y>=25 && ball.position.z<=-50){
		SC+=1//第四層配置
		Th=0
		...
		wall.rotation.x+=Math.PI*0.5
		wall.scale.x=0.375
		world2D.slCameraRR.setLabel('發現最好時機,呈現最深天機(建議多加嘗試各功能)///[已擊中3/5]:藏')
	}else if(SC===3 && ball.position.x>=-75 && ball.position.x<=75 && ball.position.y<=50 && ball.position.z<=-50){
		SC+=1//第五層配置
		Th=0
		wall.visible=true
		wall.position.z=-100
		...
		wall.scale.x=0.375
		wall.scale.y=0.2
		world2D.slCameraRR.setLabel('發現最好時機,呈現最深天機(建議多加嘗試各功能)///[已擊中4/5]:遠')
	}else if(SC===4 && ball.position.x>=-200 && ball.position.x<=-100 && ball.position.y>=25 && ball.position.y<=35 && ball.position.z<=-50){
		SC+=1
		Th=0
		wall.position.x=150
		world2D.slCameraRR.setLabel('發現最好時機,呈現最深天機(建議多加嘗試各功能)///[已擊中5/5]:讚')
	}else if(SC===5 && ball.position.x>=100 && ball.position.x<=200 && ball.position.y>=15 && ball.position.y<=25 && ball.position.z<=-50){
		Th=0
		world2D.slCameraRR.setLabel('發現最好時機,呈現最深天機(建議多加嘗試各功能)///[破表啦~~~]')
	}


	//bar
	if(Th===0){
		bar.position.x=0.5*rr*Math.cos(theta)
		bar.position.z=0.5*rr*Math.sin(theta)+50
		bar.position.y=world2D.sl01.value
	...
	if(Th===0){
		world2D.btn01.setLabel('電神,啟動')
	}else if(Th===1 || Th===10 || Th===20 || Th===30 || Th===40){
		world2D.btn01.setLabel('電神,回收')
	}
	if(SP===5){
		world2D.btn02.setLabel('獄門旋,開')
	}else if(SP===0.1){
		world2D.btn02.setLabel('獄門旋,關')
	}
...
function clickBtn(e){
	...
	if(str==='btn01'){
		...
		if(world2D.ch01.checked===false && world2D.ch02.checked===false){
			world2D.sl01.value+=2.5
		}else if(world2D.ch01.checked===false && world2D.ch02.checked===true){
			Z-=10
		}
	}else if(str==='btnDown'){
		if(world2D.ch01.checked===false && world2D.ch02.checked===false){
