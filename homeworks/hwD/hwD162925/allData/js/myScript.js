...
var bar2;
var rr=20,rrr=5;
var theta=0,omega=0.2,theta2=0,omega2=2
var ti=0.1;
var mode=1;
var groundd;
var line;
...
//B.定義init
function init(){
	word=new TEACHER.ObjTextPlane(90,15,"162925 陳彥誠",'z',)
	scene.add(word)
	word.position.y=30
	word.position.z=-50
	world2D.sl01.setLabel("圓半徑")
	world2D.sl02.setLabel("大圓omega")
	world2D.sl03.setLabel("線條顏色")
	...
	world2D.sl02.value=0.2
	world2D.sl03.visible=false
    world2D.ch01.setLabel("操控小圓")
	world2D.ch02.setLabel("跟隨小球")
	world2D.ch02.on("click",aa)
	world2D.ch01.on("click",little)
	world2D.btn01.setLabel("擦掉")
	world2D.btn02.setLabel("俯視")
	world2D.btn01.on("click",reset)
	world2D.btn02.on("click",up)

	line=new TEACHER.Line(0x7cfc00,Infinity)
	ball=new TEACHER.ObjSphere(2,0x165615)
	bar=new TEACHER.ObjCylinder(0.3,rr,0xffff00,true,"x",2)
	bar2=new TEACHER.ObjCylinder(0.3,rrr,0x0000ff,true,"x",2)
	center=new TEACHER.ObjCylinder(1,5,0xff0000)
	center2=new TEACHER.ObjCylinder(1,5,0xff0000)
	groundd=new TEACHER.ObjPlane(100,100,0xffffff,"y",0)
	scene.add(ball,bar,center,center2,bar2,groundd,line)
	groundd.position.y=0
	groundd.position.z=0
	groundd.position.x=0
	ball.position.y=bar.position.y=center.position.y=2
	ball.position.y=bar2.position.y=center2.position.y=2
...
//C.定義tick                                                                                                                       
function tick(){
	color=world2D.sl03.value
	if(color === 1){
		ball.mat.color.setHex(0x7cfc00)
	}

	if(md==1){
		omega=world2D.sl02.value
		rr=world2D.sl01.value
	    bar.scale.x=world2D.sl01.value/20
	}else{
		
		omega2=world2D.sl02.value
		rrr=world2D.sl01.value
	    bar2.scale.x=world2D.sl01.value/5
	}
	theta2+=omega2*ti
    ...
    theta+=omega*ti
    center2.position.x=rr*Math.cos(theta)
    center2.position.z=rr*Math.sin(theta)

	bar.position.x=0.5*rr*Math.cos(theta)
    bar.position.z=0.5*rr*Math.sin(theta)
    bar.rotation.y=-theta
    
	theta2+=omega2*ti

    ball.position.x=rrr*Math.cos(theta2)+center2.position.x
    ball.position.z=rrr*Math.sin(theta2)+center2.position.z

	bar2.position.x=0.5*rrr*Math.cos(theta2)+center2.position.x
    bar2.position.z=0.5*rrr*Math.sin(theta2)+center2.position.z
    bar2.rotation.y=-theta2

    line.addPoint(ball.position.x,ball.position.y-1,ball.position.z)
...
function little(){
	if(world2D.ch01.checked==true){
		world2D.sl01.setLabel("小圓半徑")
	    world2D.sl02.setLabel("小圓omega")
		md=0
		world2D.sl01.maximum=10
	    world2D.sl01.minimum=2
	    world2D.sl02.maximum=8
	    world2D.sl02.minimum=-8
	    world2D.sl02.digitN=1
		world2D.sl01.value=rrr
		world2D.sl02.value=omega2
	}else{
		world2D.sl01.setLabel("大圓半徑")
	    world2D.sl02.setLabel("大圓omega")
		md=1
		world2D.sl01.maximum=40
	    world2D.sl01.minimum=10
	    ...
	    world2D.sl02.minimum=-5
	    world2D.sl02.digitN=1
		world2D.sl01.value=rr
		world2D.sl02.value=omega
	}
}
