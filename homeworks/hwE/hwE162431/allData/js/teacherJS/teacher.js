...
//mainAnimation DIV
$('#mainAnimation').css('position', 'absolute').css('top', '0px').css('left', '0px').css('width', '1600px').css('height', '900px');
...
(function () {
  ...
  function myResize() {
    ...
    $('#mainAnimation')
      .css('width', windowScale * oldWidth)
      .css('height', windowScale * oldHeight);
    ...
    for (var i = 0; i < len; i++) {
      ...
      $(domElement)
        .css({
          '-webkit-transform': 'scale(' + TwindowScale + ')',
...
function myMouseMove(e) {
  if (world3D.camera.isDraging && world3D.cameraDragAndRotate) {
    ...
    while (world3D.cameraPhi > Math.PI) {
    ...
    if (world3D.cameraTheta > 0.9999 * Math.PI) {
      world3D.cameraTheta = 0.9999 * Math.PI;
    }
    if (world3D.cameraTheta < 0.0001 * Math.PI) {
      world3D.cameraTheta = 0.0001 * Math.PI;
    }
