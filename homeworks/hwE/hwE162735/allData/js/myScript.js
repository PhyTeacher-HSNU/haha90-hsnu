...
function clickBtn(e)
	{
		...
		if(str==='btn02' && isplay==true)
		{
			
			isplay=false

		}else
		if(str==='btn02' && isplay==false)
		{
			isplay=true
...
function init(){
	...
	pts1 = new TEACHER.Points(0xdd0000,1 ,10000)
	for(var i = 0; i<nn*nn;i++)
	{
		pts1.addPoint((i%nn-0.5*nn)*1.2,30,Math.floor(i/nn-0.5*nn)*1.2)

	}
	pts1.update()
	scene.add(pts1)

	pts2 = new TEACHER.Points(0x0000dd,1 ,10000)
	for(var i = 0; i<nn*nn;i++)
	{
		pts2.addPoint((i%nn-0.5*nn)*1.2,-30,Math.floor(i/nn-0.5*nn)*1.2)
	...
	pts3 = new TEACHER.Points(0xdd00dd,2 ,10000)
	for(var i = 0; i<nn*nn;i++)
	{
		pts3.addPoint((i%nn-0.5*nn)*1.2,0,Math.floor(i/nn-0.5*nn)*1.2)
	...
	world2D.sl01.minimum = 5;
	world2D.sl01.maximum = 30;
	world2D.sl01.value = 20;

	world2D.sl02.setLabel('距離');
	world2D.sl02.minimum = 0;
	world2D.sl02.maximum = 70;
	world2D.sl02.value = 20;

	world2D.sl03.setLabel('強度');
	world2D.sl03.minimum = 1;
	world2D.sl03.maximum = 5;
	world2D.sl03.value = 2;

	world2D.ch01.setLabel('3 in 1');

	world2D.btn01.setLabel('step');
	world2D.btn02.setLabel('start/stop');

	center1 = new TEACHER.ObjBox(1,1,1,0x00dddd,0)
	center2 = new TEACHER.ObjBox(1,1,1,0x00dddd,0)
	scene.add(center1)
	scene.add(center2)
...
function tick(){
	...
	{   
		tt += dt
		lamda = world2D.sl01.value
		for(var i=0;i<nn*nn;i++)
		{
			var pt = pts3.getPoint(i)
			var xx = pt.x
			var zz = pt.z
			var xx1 = Math.abs(pt.x-(+world2D.sl02.value/2))
			var zz1 = pt.z
			var rr1 = Math.sqrt(xx1*xx1+zz1*zz1)
			var xx2 = Math.abs(pt.x-(-world2D.sl02.value/2))
			var zz2 = pt.z 
			var rr2 = Math.sqrt(xx2*xx2+zz2*zz2)
			var yy = world2D.sl03.value*Math.sin(2*Math.PI*rr1/lamda - 2*Math.PI*tt/10)+world2D.sl03.value*Math.sin(2*Math.PI*rr2/lamda - 2*Math.PI*tt/10)
	
			pts3.setPoint(i,xx,yy,zz)
		...
		for(var i=0;i<nn*nn;i++)
		{
			...
			var yy = world2D.sl03.value*Math.sin(2*Math.PI*rr1/lamda - 2*Math.PI*tt/10)
	
			pts1.setPoint(i,xx,yy+yy1,zz)
	
			center1.position.x = world2D.sl02.value/2
			center1.position.y = yy1+2*Math.sin(2*Math.PI*(0)/lamda - 2*Math.PI*tt/10)
		...
		for(var i=0;i<nn*nn;i++)
		{
			...
			var yy = world2D.sl03.value*Math.sin(2*Math.PI*rr2/lamda - 2*Math.PI*tt/10)
	
			pts2.setPoint(i,xx,yy+yy2,zz)
	
			center2.position.x = -world2D.sl02.value/2
			center2.position.y = yy2+2*Math.sin(2*Math.PI*(0)/lamda - 2*Math.PI*tt/10)
