...
const CONST_TEXT2 = '請使用方向按鈕控制球進入紅圈';
let ball;
const ballRadius = 10;
let ballVx = 0;
let ballVz = 0;
...
function init() {
  ...
  circle = new TEACHER.ObjCylinder(ballRadius * 1.5, 0.25, 0xff2020, false, 'y', 2);
  circle.position.y = 1;
  world3D.scene.add(circle);

  const constText = new TEACHER.ObjTextPlane(160, 20, CONST_TEXT, 'y', 0xffffff);
  constText.position.y = 40;
  constText.position.z = -50;
  constText.rotateX(Math.PI / 2);
  world3D.scene.add(constText);

  scoreText = new TEACHER.ObjTextPlane(160, 20, '', 'y', 0xffffff);
  scoreText.position.y = 20;
  scoreText.position.z = -50;
  scoreText.rotateX(Math.PI / 2);
  world3D.scene.add(scoreText);

  const constText2 = new TEACHER.ObjTextPlane(160, 10, CONST_TEXT2, 'y', 0xffffff);
  constText2.position.y = 60;
  constText2.position.z = -50;
...
function isBallAndCircleCollide() {
  return ball.position.distanceTo(circle.position) < ballRadius * 1.5;
}
function moveCirclePos() {
  const x = Math.random() * 100 - 50;
  const z = Math.random() * 100 - 50;
  circle.position.set(x, 1, z);
  if (isBallAndCircleCollide()) {
    moveCirclePos();
  }
}
...
function tick() {
  ...
  scoreText.setText(`分數: ${score} 倒數時間: ${Math.ceil(timeLeft / fps)}秒`);
  //3D
  world3D.render();
  //2D
