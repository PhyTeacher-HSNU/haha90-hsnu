...
//world3D
var world3D={};
world3D.cameraRR=world3D.cameraRRTarget=20;
world3D.cameraTheta=world3D.cameraThetaTarget=75*Math.PI/180;
world3D.cameraPhi=world3D.cameraPhiTarget=0*Math.PI/180;
//Three.js
world3D.scene = new THREE.Scene();
var scene = world3D.scene;
world3D.camera = new THREE.PerspectiveCamera( 45, 1600 / 900, 1, 10000 );
...
function myMouseMove(e){
	if(world3D.camera.isDraging && world3D.cameraDragAndRotate){
		...
		world3D.cameraPhiTarget-=0.005*(nx-world3D.camera.oldMouseX);
		while(world3D.cameraPhiTarget>Math.PI){world3D.cameraPhiTarget-=2*Math.PI;}
		while(world3D.cameraPhiTarget<-Math.PI){world3D.cameraPhiTarget+=2*Math.PI;}

		world3D.cameraThetaTarget-=0.005*(ny-world3D.camera.oldMouseY);
		if(world3D.cameraThetaTarget>0.9999*Math.PI){world3D.cameraThetaTarget=0.9999*Math.PI;}
		if(world3D.cameraThetaTarget<0.0001*Math.PI){world3D.cameraThetaTarget=0.0001*Math.PI;}
...
world3D.render = function() {
	//設定攝影機位置
	world3D.cameraRRTarget=world2D.slCameraRR.value;

	world3D.cameraRR=0.9*world3D.cameraRR+0.1*world3D.cameraRRTarget;
	world3D.cameraTheta=0.9*world3D.cameraTheta+0.1*world3D.cameraThetaTarget;
	world3D.cameraPhi=0.9*world3D.cameraPhi+0.1*world3D.cameraPhiTarget;
