...
var world3D={};
world3D.cameraRR=20;
world3D.cameraTheta=0.0001*Math.PI/180;
world3D.cameraPhi=0*Math.PI/180;
//Three.js
world3D.scene = new THREE.Scene();
var scene = world3D.scene;
world3D.camera = new THREE.PerspectiveCamera( 45/3, 1600 / 900, 1, 100000 );
...
function myMouseMove(e){
	if(world3D.camera.isDraging && world3D.cameraDragAndRotate){
		...
		while(world3D.cameraPhi>0.001*Math.PI){world3D.cameraPhi=0.001*Math.PI;}
		while(world3D.cameraPhi<-0.001*Math.PI){world3D.cameraPhi=-0.001*Math.PI;}

		world3D.cameraTheta-=0.005*(ny-world3D.camera.oldMouseY);
		if(world3D.cameraTheta>0.48*Math.PI){world3D.cameraTheta=0.48*Math.PI;}
...
world3D.render = function() {
	//設定攝影機位置
	world3D.cameraRR=world2D.slCameraRR.value*3;
